...
interface RenderableSduiDefinition {
    ...
    @Composable fun RenderComponent(node: Component, context: SduiRenderContext, children: @Composable () -> Unit): Boolean = false
    @Composable fun RenderSection(node: Section, context: SduiRenderContext, children: @Composable () -> Unit): Boolean = false
    @Composable fun RenderGroup(node: Group, context: SduiRenderContext, children: @Composable () -> Unit): Boolean = false
    @Composable fun RenderElement(node: Element, context: SduiRenderContext): Boolean = false
}
...
class SduiRendererDispatcher(private val registry: SduiRegistry) {
    @Composable
    ...
    private fun dispatch(
        ...
        val registered: SduiDefinition<*> = registry.find(kind, type) ?: run {
            onFailure(SduiRenderFailure.MissingDefinition(kind, type, path)); return
        }
        val renderable = registered as? RenderableSduiDefinition ?: run {
            onFailure(SduiRenderFailure.DefinitionIsNotRenderable(kind, type, path)); return
        }
